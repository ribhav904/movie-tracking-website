...
export default function SettingsPage() {
  ...
  return <div className="page-stack settings-page">
    ...
    <section className="settings-section panel"><div><p className="eyebrow">Account</p><h2>{email}</h2><p>{state === "preview" ? "Preview mode is local only. Configure Supabase to use a personal account." : "Your password and session are managed by Supabase Auth."}</p></div><div className="settings-actions"><button className="button button--secondary"><Download size={16} /> Export my data</button>{state !== "preview" && <button className="button button--quiet" onClick={() => void signOut()}><LogOut size={16} /> Sign out</button>}</div></section>

...
import { Heart, LayoutGrid, ListFilter, Rows3, Search } from "lucide-react";
...
export default function LibraryPage() {
  ...
  const entries = useMemo(() => (library.data?.items ?? []).filter((entry) => (status === "all" || entry.status === status) && (!search || entry.media?.title.toLowerCase().includes(search.toLowerCase()))), [library.data, search, status]);
  ...
  return <div className="page-stack">
    ...
    <section className="library-toolbar">
      ...
      <div className="toolbar-actions"><label className="search-field search-field--small"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filter your library" /></label><button className="icon-button" aria-label="Filter library"><ListFilter size={17} /></button><div className="view-switch"><button className={view === "grid" ? "is-active" : ""} aria-label="Grid view" onClick={() => setView("grid")}><LayoutGrid size={16} /></button><button className={view === "list" ? "is-active" : ""} aria-label="List view" onClick={() => setView("list")}><Rows3 size={16} /></button></div></div>
    </section>

...
export default function DiscoverPage() {
  ...
  const path = useMemo(() => submittedSearch ? `/media/search?media_type=${mediaType}&query=${encodeURIComponent(submittedSearch)}&page=1` : `/media/discover?media_type=${mediaType}&page=1`, [mediaType, submittedSearch]);
  const results = useQuery({ queryKey: ["discover", mediaType, submittedSearch], queryFn: () => apiRequest<Page<MediaSummary>>(path) });
  ...
  return <div className="page-stack">
    ...
    <section className="discover-toolbar">
      <div className="segmented-control" aria-label="Media type">{mediaTypes.map((item) => <button key={item.value} className={mediaType === item.value ? "is-active" : ""} onClick={() => { setMediaType(item.value); setSubmittedSearch(""); }}>{item.label}</button>)}</div>
      <form className="search-field" onSubmit={(event) => { event.preventDefault(); setSubmittedSearch(draftSearch.trim()); }}><Search size={18} /><input value={draftSearch} onChange={(event) => setDraftSearch(event.target.value)} placeholder={`Search ${typeLabel}`} /><button className="search-submit" type="submit">Search</button></form>
    ...
    <section className="content-section">
      <div className="section-heading"><div><p className="eyebrow">{submittedSearch ? `Results for “${submittedSearch}”` : "A starting point"}</p><h2>{submittedSearch ? "Matches from your selected source" : `Explore ${typeLabel}`}</h2></div><span className="result-count">{visibleResults.length} items</span></div>
